...
alter table public.client_profiles
  ...
  check (client_type in ('new', 'regular', 'ambassador', 'vip', 'team'));
...
begin
  ...
  if p_client_type not in ('new', 'regular', 'ambassador', 'vip', 'team') then
    raise exception 'Invalid client_type';
  end if;
...
where a.client_id is null
  and (
    (nullif(lower(trim(a.client_email)), '') is not null and c.normalized_email = lower(trim(a.client_email)))
    or (public.normalize_phone(a.client_phone) is not null and c.normalized_phone = public.normalize_phone(a.client_phone))
    or lower(trim(a.client_name)) = lower(trim(c.first_name || ' ' || c.last_name))
  );
...
revoke all on function public.hub_update_appointment_details(uuid, text, text, text, text, integer, integer) from public;
revoke all on function public.hub_update_appointment_details(uuid, text, text, text, text, integer, integer) from anon;
grant execute on function public.hub_update_appointment_details(uuid, text, text, text, text, integer, integer) to authenticated;
