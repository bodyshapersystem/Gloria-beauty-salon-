    ...
    <>
      <header className="sticky top-0 z-50 bg-ivory/90 backdrop-blur-md border-b border-taupe/25">
        <nav className="max-w-[1220px] mx-auto flex items-center justify-between px-5 md:px-8 py-3 md:py-4">
          ...
          <button aria-label={open ? "Cerrar menú" : "Abrir menú"} aria-expanded={open} onClick={() => setOpen((v) => !v)} className="xl:hidden inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-espresso hover:bg-champagne/15 transition-colors">{open ? <X size={24} strokeWidth={1.5} /> : <Menu size={26} strokeWidth={1.5} />}</button>
        </nav>
      </header>
