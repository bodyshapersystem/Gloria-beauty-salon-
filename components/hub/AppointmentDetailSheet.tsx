...
export function AppointmentDetailSheet({appointmentId,onClose,onSaved}:{appointmentId:string;onClose:()=>void;onSaved:()=>void}){
  ...
  const [extraEligibleStaff,setExtraEligibleStaff]=useState<{id:string;name:string}[]>([]);
  ...
  useEffect(()=>{
    if(!extraServiceId){setExtraEligibleStaff([]);setExtraStaffId("");setExtraPrice("");return}
    (async()=>{
      const {data}=await supabase.from("staff_services").select("staff:staff_id(id,name)").eq("service_id",extraServiceId);
      const list=((data as any[])||[]).map(x=>firstRelation(x.staff)).filter(Boolean) as {id:string;name:string}[];
      setExtraEligibleStaff(list);
      setExtraStaffId(list.length===1?list[0].id:(appointment&&list.some(s=>s.id===appointment.staff_id)?appointment.staff_id:""));
    })();
    const svc=services.find(s=>s.id===extraServiceId);
    setExtraPrice(svc?centsToInput(parsePrice(svc.price_label)):"");
  },[extraServiceId]);
  ...
  async function addExtra(){
    if(!appointment||!extraService||!extraStaffId)return;
    setSaving(true);setMessage(null);
    const {data:newId,error}=await supabase.rpc("hub_create_appointment",{
      p_service_id:extraService.id,
      p_staff_id:extraStaffId,
      p_start_at:appointment.start_at,
      p_client_name:appointment.client_name,
      p_client_phone:appointment.client_phone||"",
      p_client_email:appointment.client_email,
      p_notes_internal:`Servicio agregado desde la cita de ${appointment.staff?.name||"otra profesional"}.`,
      p_source:"gloria_hub"
    });
    if(error){setSaving(false);setMessage(`No se pudo agregar el servicio: ${error.message}`);return;}
    await supabase.rpc("hub_update_appointment_status",{p_appointment_id:newId,p_status:"completed"});
    const priceCents=extraPrice.trim()===""?null:inputToCents(extraPrice);
    if(priceCents!==null){
      await supabase.rpc("hub_update_appointment_details",{
        ...
        p_deposit_cents:null
      });
    }
    setSaving(false);
    const staffName=extraEligibleStaff.find(s=>s.id===extraStaffId)?.name||"la profesional";
    setMessage(`${extraService.name} agregado y atribuido a ${staffName} — cuenta en su propio progreso.`);
    setExtraServiceId("");
    await load();onSaved();
  }
    ...
    <aside className="h-full w-full max-w-[540px] overflow-y-auto bg-[#FBF8F3] p-5 md:p-7" onClick={e=>e.stopPropagation()}>
      ...
      {!appointment?<p className="mt-8 text-[12px] text-taupe">Abriendo detalle de la cita...</p>:<>
        ...
        <Section title="Agregar más a la cita">
          <select value={extraServiceId} onChange={e=>setExtraServiceId(e.target.value)} className="w-full rounded-[15px] border border-[#D8C8BC] bg-white px-4 py-3.5 text-[12px] outline-none"><option value="">Seleccionar servicio extra</option>{services.map(s=><option key={s.id} value={s.id}>{s.name}{s.price_label?` · ${s.price_label}`:""}</option>)}</select>
          {extraServiceId&&<>
            <label className="block"><span className="mb-1.5 block text-[8px] uppercase tracking-[.14em] text-taupe">¿Con quién fue?</span><select value={extraStaffId} onChange={e=>setExtraStaffId(e.target.value)} className="w-full rounded-[15px] border border-[#D8C8BC] bg-white px-4 py-3.5 text-[12px] outline-none"><option value="">Seleccionar profesional</option>{extraEligibleStaff.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select>{extraEligibleStaff.length===0&&<span className="mt-1.5 block text-[9px] text-red-700">Este servicio no tiene profesionales asignadas todavía.</span>}</label>
            <Field label="Precio de este servicio" value={extraPrice} set={setExtraPrice} inputMode="decimal" prefix="$"/>
            <button onClick={addExtra} disabled={!extraStaffId||saving} className="inline-flex w-full items-center justify-center gap-2 rounded-[15px] bg-[#6F3642] px-4 py-3 text-[9px] uppercase tracking-[0.12em] text-white disabled:opacity-40"><Plus size={14}/>Agregar servicio</button>
          </>}
          <p className="text-[9px] leading-relaxed text-taupe">Esto crea una cita completada aparte, atribuida a quien la hizo — así cuenta en su propio progreso e ingresos, no en los de esta cita.</p>
        </Section>
