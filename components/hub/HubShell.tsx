...
export function HubShell({children}:{children:React.ReactNode}){
  ...
  return <div className="min-h-screen bg-[#F7F3ED] text-espresso md:grid md:grid-cols-[196px_1fr] pb-[82px] md:pb-0">
    <aside className="hidden md:flex bg-[#34261F] px-3 py-4 flex-col sticky top-0 h-screen text-ivory overflow-hidden">
      ...
      <nav className="mt-4 space-y-1">{nav.map(item=>{const Icon=item.icon;const active=pathname===item.href||pathname.startsWith(item.href+"/");return <Link key={item.href} href={item.href} className={`flex items-center gap-2.5 rounded-[11px] px-3 py-2.5 text-[9px] font-medium uppercase tracking-[0.1em] transition-all ${active?"bg-white/12 text-white":"text-ivory/65 hover:bg-white/7 hover:text-white"}`}><Icon size={15} strokeWidth={1.55}/><span>{item.label}</span>{active&&<span className="ml-auto h-1.5 w-1.5 rounded-full bg-champagne"/>}</Link>})}</nav>
      <div className="mt-auto border-t border-white/10 pt-3">
        <Link href="/hub/profile" className="mb-2 flex items-center gap-3 rounded-[14px] bg-white/[.06] px-3 py-3">
          <span className="h-8 w-8 shrink-0 overflow-hidden rounded-full border border-white/15 bg-white/10">{identity.photo_url?<img src={identity.photo_url} alt={identity.name} className="h-full w-full object-cover"/>:<span className="grid h-full place-items-center font-serif text-[15px] text-champagne">{identity.name[0]}</span>}</span>
          <span className="min-w-0"><span className="block text-[7px] uppercase tracking-[.15em] text-ivory/45">{greeting}</span><span className="mt-0.5 block truncate font-serif text-[15px] text-ivory">{identity.name}</span></span>
        </Link>
        <Link href="/hub/profile" className="flex items-center gap-2.5 rounded-[11px] px-3 py-2.5 text-[9px] font-medium uppercase tracking-[0.1em] text-ivory/58 hover:bg-white/7 hover:text-white"><UserRound size={15}/> Mi Perfil</Link>{isOwner&&<Link href="/hub/settings/appointments" className="flex items-center gap-2.5 rounded-[11px] px-3 py-2.5 text-[9px] font-medium uppercase tracking-[0.1em] text-ivory/58 hover:bg-white/7 hover:text-white"><Settings size={15}/> Configuración</Link>}<button onClick={logout} className="flex w-full items-center gap-2.5 rounded-[11px] px-3 py-2.5 text-[9px] font-medium uppercase tracking-[0.1em] text-ivory/48 hover:bg-white/7 hover:text-white"><LogOut size={15}/> Cerrar sesión</button></div>
    </aside>

    <div className="min-w-0">
      <header className="sticky top-0 z-40 border-b border-champagne/20 bg-[#F7F3ED]/94 backdrop-blur-xl"><div className="h-[58px] px-4 md:px-7 lg:px-9 flex items-center justify-between gap-4 max-w-[1580px] mx-auto"><div className="md:hidden"><Logo className="h-10 w-auto"/></div><div className="hidden md:flex items-center gap-3"><span className="h-8 w-8 overflow-hidden rounded-full border border-champagne/40 bg-[#EADDD4]">{identity.photo_url?<img src={identity.photo_url} alt={identity.name} className="h-full w-full object-cover"/>:<span className="grid h-full place-items-center font-serif text-[14px] text-mocha">{identity.name[0]}</span>}</span><div><p className="text-[7px] font-medium uppercase tracking-[0.22em] text-taupe">{greeting}</p><p className="mt-0.5 font-serif text-[15px] leading-none text-mocha">{identity.name}</p></div></div><div className="flex items-center gap-2"><Link href="/hub/calendar?new=1" className="hidden sm:inline-flex items-center gap-2 rounded-full bg-[#6F3642] px-4 py-2.5 text-[8px] font-medium uppercase tracking-[0.16em] text-white"><Plus size={14}/> Nueva cita</Link><button className="md:hidden grid h-10 w-10 place-items-center rounded-full border border-champagne/35 bg-white/60" onClick={()=>setMoreOpen(true)} aria-label="Más opciones"><Menu size={21}/></button></div></div></header>
