...
import { CalendarDays, CalendarRange, LayoutDashboard, Menu, PackageCheck, Scissors, Settings, ShoppingBag, Sparkles, UserRound, UsersRound, X } from "lucide-react";
import { Logo } from "@/components/brand/Logo";
import { supabase } from "@/lib/supabase/client";

type HubUser={role:"owner"|"admin"|"staff";staff_id:string|null};
const nav=[
  {label:"Dashboard",href:"/hub",icon:LayoutDashboard,adminOnly:false},
  {label:"Appointments",href:"/hub/appointments",icon:CalendarDays,adminOnly:false},
  {label:"Calendar",href:"/hub/calendar",icon:CalendarRange,adminOnly:false},
  {label:"Clients",href:"/hub/clients",icon:UsersRound,adminOnly:false},
  {label:"Beauty Intelligence",href:"/hub/intelligence",icon:Sparkles,adminOnly:false},
  {label:"Team",href:"/hub/team",icon:UserRound,adminOnly:false},
  {label:"Services",href:"/hub/services",icon:Scissors,adminOnly:true},
  {label:"Products",href:"/hub/products",icon:ShoppingBag,adminOnly:true},
  {label:"Orders",href:"/hub/orders",icon:PackageCheck,adminOnly:true},
  {label:"Settings",href:"/hub/settings/appointments",icon:Settings,adminOnly:true},
];
...
export function HubShell({children}:{children:React.ReactNode}){
  ...
  if(!user)return null;
  const visibleNav=nav.filter(item=>!item.adminOnly||user.role!=="staff");
  ...
  return <div className="min-h-screen bg-ivory text-espresso md:grid md:grid-cols-[250px_1fr]">
    <aside className="hidden md:flex min-h-screen border-r border-champagne/25 bg-white/35 px-5 py-6 flex-col sticky top-0 h-screen">
      ...
      <div className="mt-6"><p className="text-[9px] uppercase tracking-[0.28em] text-mocha">Gloria Hub</p><p className="mt-2 font-serif text-[23px] leading-none">Business, beautifully organized.</p></div>
    ...
    <div className="min-w-0">
      <header className="md:hidden sticky top-0 z-40 h-[72px] border-b border-champagne/25 bg-ivory/95 px-5 flex items-center justify-between backdrop-blur-md"><Logo className="h-12 w-auto"/><button onClick={()=>setOpen(v=>!v)} aria-label="Open Hub menu">{open?<X size={25}/>:<Menu size={27}/>}</button></header>
      {open&&<div className="md:hidden fixed inset-0 top-[72px] z-50 bg-espresso/40" onClick={()=>setOpen(false)}><div className="w-[86%] max-w-[360px] h-full bg-ivory p-5 overflow-y-auto" onClick={e=>e.stopPropagation()}><p className="text-[9px] uppercase tracking-[0.25em] text-mocha">Gloria Hub</p><nav className="mt-4">{visibleNav.map(item=>{const Icon=item.icon;return <Link onClick={()=>setOpen(false)} key={item.href} href={item.href} className="flex items-center gap-3 border-b border-champagne/20 py-3.5 text-[13px]"><Icon size={17}/>{item.label}</Link>})}</nav></div></div>}
