...
type Appointment={id:string;client_name:string;start_at:string;end_at:string;status:string;service_id:string;staff_id:string;service_name:string|null;staff_name:string|null;service_category:string|null};
...
function AppointmentCard({a,onOpen}:{a:Appointment;onOpen:()=>void}){return <button onClick={onOpen} style={{backgroundImage:categoryBg(a.service_name,a.service_category)}} className="w-full rounded-[20px] p-4 text-left text-white shadow-[0_8px_22px_rgba(74,29,36,.10)]"><div className="flex gap-4"><div className="w-[62px] shrink-0"><p className="font-serif text-[20px]">{fmtTime(a.start_at)}</p><p className="mt-1 text-[8px] text-white/60">{minutes(a)} min</p></div><div className="min-w-0 flex-1 border-l border-white/15 pl-4"><p className="font-serif text-[23px] leading-none truncate">{a.client_name}</p><p className="mt-2 text-[10px] text-white/80">{a.service_name||"Cita"}</p><p className="mt-1 text-[9px] text-white/55">con {a.staff_name||"Equipo"}</p></div></div></button>}
