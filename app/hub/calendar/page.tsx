...
export default function HubCalendarPage(){
  ...
  return <div>
    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5">
      <div><p className="text-[10px] uppercase tracking-[0.28em] text-mocha">Gloria Hub</p><h1 className="mt-2 font-serif text-[46px] md:text-[58px] leading-none">Calendar</h1><p className="mt-3 text-[13px] text-taupe">See the salon rhythm by day, week or month.</p></div>
    ...
    <div className="mt-7 flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-2"><button onClick={()=>setAnchor(move(anchor,view,-1))} className="h-10 w-10 rounded-full border border-champagne/35 flex items-center justify-center"><ChevronLeft size={17}/></button><button onClick={()=>setAnchor(new Date())} className="rounded-full border border-champagne/35 px-4 py-2.5 text-[9px] uppercase tracking-[0.13em]">Today</button><button onClick={()=>setAnchor(move(anchor,view,1))} className="h-10 w-10 rounded-full border border-champagne/35 flex items-center justify-center"><ChevronRight size={17}/></button><p className="ml-2 font-serif text-[24px]">{label(anchor,view)}</p></div>
      <div className="flex rounded-full border border-champagne/35 p-1">{(["day","week","month"] as View[]).map(v=><button key={v} onClick={()=>setView(v)} className={`rounded-full px-4 py-2 text-[9px] uppercase tracking-[0.12em] ${view===v?"bg-espresso text-ivory":"text-taupe"}`}>{v}</button>)}</div>
    </div>

    {loading?<p className="mt-8 text-[13px] text-taupe">Loading calendar...</p>:view==="month"?<Month days={days} items={visible}/>:<>
      <div className="hidden md:block mt-6 overflow-x-auto rounded-[24px] border border-champagne/30 bg-white/35">
        <div className="min-w-[900px] grid" style={{gridTemplateColumns:`130px repeat(${Math.max(staff.length,1)},minmax(150px,1fr))`}}>
          <div className="border-b border-r border-champagne/25 p-4 text-[9px] uppercase tracking-[0.14em] text-taupe">{view==="day"?fmtDay(days[0]):"Professional"}</div>
      ...
      <div className="md:hidden mt-6 space-y-6">{days.map(d=><section key={d.toISOString()}><p className="text-[9px] uppercase tracking-[0.18em] text-mocha">{fmtDay(d)}</p><div className="mt-3 space-y-2">{visible.filter(a=>sameDay(new Date(a.start_at),d)).sort(sortTime).map(a=><AppointmentCard key={a.id} a={a}/>)}{visible.filter(a=>sameDay(new Date(a.start_at),d)).length===0&&<p className="text-[12px] text-taupe">No appointments.</p>}</div></section>)}</div>
    </>}
  </div>
}

function AppointmentCard({a}:{a:Appointment}){return <Link href={`/hub/appointments?appointment=${a.id}`} className="block rounded-[16px] border border-champagne/30 bg-blush/25 p-4"><p className="text-[9px] text-taupe">{fmtTime(a.start_at)} · {human(a.status)}</p><p className="mt-1 font-serif text-[23px] leading-none">{a.client_name}</p><p className="mt-2 text-[11px] text-mocha">{a.service?.name||"Appointment"} · {a.staff?.name||""}</p></Link>}
function Month({days,items}:{days:Date[];items:Appointment[]}){return <div className="mt-6 grid grid-cols-2 md:grid-cols-7 overflow-hidden rounded-[24px] border border-champagne/30 bg-white/35">{days.map(d=>{const list=items.filter(a=>sameDay(new Date(a.start_at),d));return <div key={d.toISOString()} className="min-h-[130px] border-r border-b border-champagne/20 p-3"><p className="text-[10px] text-taupe">{d.getDate()}</p><div className="mt-2 space-y-1">{list.slice(0,3).map(a=><p key={a.id} className="rounded bg-ivory px-2 py-1 text-[9px] truncate">{fmtTime(a.start_at)} {a.client_name}</p>)}{list.length>3&&<p className="text-[9px] text-mocha">+{list.length-3} more</p>}</div></div>})}</div>}
...
function label(d:Date,view:View){return view==="month"?d.toLocaleDateString("en-US",{month:"long",year:"numeric"}):d.toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})}
function sameDay(a:Date,b:Date){return a.getFullYear()===b.getFullYear()&&a.getMonth()===b.getMonth()&&a.getDate()===b.getDate()}
function sortTime(a:Appointment,b:Appointment){return +new Date(a.start_at)-+new Date(b.start_at)}
function fmtTime(v:string){return new Date(v).toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",timeZone:"America/New_York"})}
function fmtDay(d:Date){return d.toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}
