...
function ClientCard({c,canEdit,canDelete,onEdit,onDelete}:{c:Client;canEdit:boolean;canDelete:boolean;onEdit:()=>void;onDelete:()=>void}){return <div className="relative rounded-[20px] border border-champagne/30 bg-white/70 p-4 shadow-[0_6px_20px_rgba(52,38,31,.035)]"><Link href={`/hub/clients/${c.id}`} className="block"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><p className="font-serif text-[24px] leading-none">{c.first_name} {c.last_name}</p>{c.client_type==="ambassador"&&<span className="rounded-full bg-espresso px-2 py-0.5 text-[7px] uppercase tracking-[0.1em] text-champagne">Ambassador</span>}{c.client_type==="team"&&<span className="rounded-full bg-mocha px-2 py-0.5 text-[7px] uppercase tracking-[0.1em] text-ivory">Team</span>}</div><p className="mt-2 text-[10px] text-taupe">{c.phone||"Sin teléfono"}</p><p className="mt-1 text-[9px] text-taupe">{c.email||"Sin email"}</p></div><span className="text-[24px] text-champagne">›</span></div><div className="mt-4 flex flex-wrap gap-2"><span className="rounded-full bg-[#EFE5DC] px-2.5 py-1 text-[8px] text-mocha">{accessLabels[c.access_status]||c.access_status}</span><span className="rounded-full bg-blush/35 px-2.5 py-1 text-[8px] text-mocha">{beautyLabels[c.beauty_state]||c.beauty_state}</span></div></Link>{(canEdit||canDelete)&&<div className="mt-3 flex gap-2 border-t border-champagne/20 pt-3">{canEdit&&<button onClick={onEdit} className="inline-flex items-center gap-2 rounded-full border border-champagne/35 bg-white px-3 py-2 text-[8px] text-mocha"><Pencil size={12}/> Editar</button>}{canDelete&&<button onClick={onDelete} className="inline-flex items-center gap-2 rounded-full border border-[#7B3C48]/20 bg-white px-3 py-2 text-[8px] text-[#7B3C48]"><Trash2 size={12}/> Eliminar</button>}</div>}</div>}
