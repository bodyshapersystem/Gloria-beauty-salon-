...
export default function HubHome(){
  ...
  if(loading)return <div className="py-20 text-center"><p className="font-serif text-[30px]">Preparing your day...</p><p className="mt-2 text-[11px] text-taupe">Loading Gloria Hub</p></div>;
  ...
  return <div>
    <section className="relative overflow-hidden rounded-[30px] border border-champagne/25 bg-[linear-gradient(135deg,#F3E7DF_0%,#F8F5EF_48%,#EFE4D9_100%)] px-5 py-6 md:px-8 md:py-8">
      ...
      <div className="relative flex flex-col xl:flex-row xl:items-end xl:justify-between gap-7">
        <div><p className="text-[9px] uppercase tracking-[0.28em] text-mocha">Today at Gloria</p><h1 className="mt-3 font-serif text-[46px] md:text-[64px] leading-[.92]">Everything you need,<br/><span className="italic text-mocha">right here.</span></h1><p className="mt-4 max-w-[560px] text-[12px] md:text-[13px] leading-relaxed text-taupe">Start with today. Book a client, check the agenda or jump straight into the relationship you need.</p></div>
        <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-2 gap-2 xl:w-[420px]">
          <Quick href="/hub/calendar?new=1" icon={Plus} label="New appointment" dark/><Quick href="/hub/calendar" icon={CalendarDays} label="Open calendar"/><Quick href="/hub/clients" icon={UsersRound} label="Find a client"/><Quick href="/hub/appointments" icon={Clock3} label="Manage bookings"/>
        </div>
      </div>
    </section>

    <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <MetricCard icon={CalendarDays} label="Today" value={String(upcoming.length)} sub={upcoming.length===1?"appointment on the agenda":"appointments on the agenda"}/>
      <MetricCard icon={UsersRound} label="Clients" value={String(clients)} sub="profiles in Gloria Access"/>
      <MetricCard icon={UserRoundCheck} label="Completed" value={`${completedPct}%`} sub={`${completed.length} of ${month.length} bookings this month`}/>
      <MetricCard icon={CircleDollarSign} label="Recorded this month" value={money(recordedAppointmentRevenue+ordersValue)} sub="appointments + shop totals recorded"/>
    </div>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[1.35fr_.85fr]">
      <section className="rounded-[28px] border border-champagne/25 bg-white/55 p-5 md:p-6">
        <div className="flex items-center justify-between gap-4"><div><p className="text-[8px] uppercase tracking-[0.22em] text-mocha">Your day</p><h2 className="mt-2 font-serif text-[34px] leading-none">Today’s flow</h2></div><Link href="/hub/calendar" className="rounded-full border border-champagne/40 px-4 py-2 text-[9px] uppercase tracking-[0.12em] text-mocha">Full calendar</Link></div>
        {next&&<div className="mt-5 rounded-[22px] bg-espresso p-5 text-ivory"><div className="flex items-start justify-between gap-4"><div><p className="text-[8px] uppercase tracking-[0.18em] text-champagne">Next up · {time(next.start_at)}</p><p className="mt-2 font-serif text-[30px] leading-none">{next.client_name}</p><p className="mt-2 text-[11px] text-ivory/70">{next.service?.name||"Appointment"} · {next.staff?.name||"Team"}</p></div><Status value={next.status}/></div><Link href="/hub/appointments" className="mt-5 inline-flex items-center gap-2 text-[9px] uppercase tracking-[0.14em] text-champagne">Open appointment <ChevronRight size={14}/></Link></div>}
        <div className="mt-3 space-y-2">{upcoming.length?upcoming.slice(next?1:0,6).map(a=><Link key={a.id} href="/hub/appointments" className="group flex items-center gap-3 rounded-[18px] border border-champagne/20 bg-ivory/45 px-4 py-3 transition hover:bg-blush/20"><div className="grid h-11 w-14 place-items-center rounded-[13px] bg-white/70 text-[11px] font-medium text-mocha">{time(a.start_at)}</div><div className="min-w-0 flex-1"><p className="font-serif text-[21px] leading-none truncate">{a.client_name}</p><p className="mt-1 text-[10px] text-taupe truncate">{a.service?.name||"Appointment"} · {a.staff?.name||"Team"}</p></div><Status value={a.status}/><ChevronRight size={15} className="text-taupe group-hover:text-mocha"/></Link>):<EmptyAgenda/>}</div>
      </section>

      <div className="space-y-5">
        <section className="rounded-[28px] border border-champagne/25 bg-white/55 p-5 md:p-6"><div className="flex items-center gap-2 text-mocha"><Sparkles size={16}/><p className="text-[8px] uppercase tracking-[0.2em]">Needs your attention</p></div><div className="mt-5 space-y-2"><Attention value={atRisk} label="Clients at risk" note="Perfect for a personal rebooking touch" href="/hub/clients"/><Attention value={vip} label="VIP clients" note="Your highest-value relationships" href="/hub/clients"/><Attention value={topService?.[1]||0} label={topService?topService[0]:"Top service"} note="Most booked service this month" href="/hub/appointments"/></div></section>
        <section className="rounded-[28px] bg-[linear-gradient(145deg,#6B4F43,#2E2724)] p-6 text-ivory"><TrendingUp size={18} className="text-champagne"/><p className="mt-4 text-[8px] uppercase tracking-[0.2em] text-champagne">This month</p><p className="mt-2 font-serif text-[36px] leading-none">{month.length} bookings</p><div className="mt-5 grid grid-cols-2 gap-2"><MiniStat label="Completed" value={completed.length}/><MiniStat label="Cancelled" value={month.filter(x=>x.status==="cancelled").length}/><MiniStat label="No-show" value={month.filter(x=>x.status==="no_show").length}/><MiniStat label="Active" value={month.filter(x=>["pending","confirmed","in_progress"].includes(x.status)).length}/></div></section>
      </div>
    </div>
  </div>
}
...
function EmptyAgenda(){return <div className="rounded-[20px] border border-dashed border-champagne/45 px-5 py-8 text-center"><p className="font-serif text-[27px]">Your day is clear.</p><p className="mt-2 text-[10px] text-taupe">No active appointments scheduled for today.</p><Link href="/hub/calendar?new=1" className="mt-4 inline-flex items-center gap-2 rounded-full bg-espresso px-4 py-2.5 text-[9px] uppercase tracking-[0.12em] text-ivory"><Plus size={13}/> Add appointment</Link></div>}
function Status({value}:{value:string}){return <span className="inline-flex w-fit rounded-full border border-champagne/30 bg-ivory/10 px-2.5 py-1 text-[8px] uppercase tracking-[0.09em] text-current">{human(value)}</span>}
